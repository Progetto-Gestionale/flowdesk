  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-0.5">Il tuo assistente virtuale pronto da condividere.</p>
      </div>

      {!publicId ? (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
          <p className="text-sm font-semibold text-amber-700 mb-1">⚠️ ID pubblico non configurato</p>
          <p className="text-sm text-amber-600">Vai su <Link href="/dashboard/impostazioni" className="underline font-medium">Impostazioni</Link> e imposta un ID pubblico per attivare il chatbot.</p>
        </div>
      ) : (
        <>
          {/* Status */}
          <div className="bg-green-50 border border-green-200 rounded-xl px-5 py-4 flex items-center gap-4">
            ...
            <span className="ml-auto text-xs bg-green-200 text-green-700 font-bold px-2 py-1 rounded-full">● Online</span>
          </div>

          {/* Link diretto */}
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3">
            <h2 className="font-semibold text-gray-900">Link diretto</h2>
            <p className="text-sm text-gray-500">Condividi questo link su WhatsApp, Instagram, Google Business o ovunque vuoi.</p>
            <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-100 rounded-lg px-3 py-2.5">
              <span className="text-sm text-indigo-700 font-mono flex-1 truncate">{widgetUrl}</span>
              <button onClick={() => navigator.clipboard.writeText(widgetUrl!)}
                className="text-xs text-indigo-600 font-semibold shrink-0 hover:text-indigo-800 bg-white px-3 py-1 rounded-md border border-indigo-200">
                Copia
              </button>
            </div>
          ...
          {/* Script per sito web */}
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3">
            <h2 className="font-semibold text-gray-900">Integra sul tuo sito web</h2>
            <p className="text-sm text-gray-500">Incolla questo codice nel tuo sito per mostrare il chatbot come una chat bubble.</p>
            ...
            <p className="text-xs text-gray-400">Funziona su WordPress, Wix, Squarespace e qualsiasi sito web.</p>
          </div>

          {/* QR Code placeholder */}
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3">
            <h2 className="font-semibold text-gray-900">Canali social</h2>
            <p className="text-sm text-gray-500">Usa il link diretto come destinazione dei pulsanti sui tuoi profili social.</p>
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={c.name} className="bg-gray-50 rounded-xl p-3 text-center">
