...
export default function MenuAsportoPage({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  async function inviaOrdine() {
    ...
    await fetch('/api/ordina', {
      ...
      body: JSON.stringify({ publicId, tavolo: 'Asporto', righe: carrello, note: noteOrdine }),
    })
    setInviando(false)
    setOrdinato(true)
    setCarrello([])
  ...
  if (ordinato) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="bg-white rounded-2xl p-10 text-center shadow-lg max-w-sm w-full">
        ...
        <p className="text-gray-500 text-sm mt-2">Il tuo ordine è stato ricevuto. Ti contatteremo a breve per confermare i dettagli.</p>
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-400">Asporto & Delivery</p>
          </div>
        </div>
        {categorie.length > 1 && (
          ...
          <div className="bg-white rounded-t-3xl shadow-2xl max-h-[85vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 px-5 py-3 space-y-3">
              ...
              <div className="pt-3">
                ...
                <textarea value={noteOrdine} onChange={e => setNoteOrdine(e.target.value)}
                  ...
                  rows={2} className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm focus:outline-none resize-none" />
              </div>
            </div>
            <div className="px-5 py-4 border-t border-gray-100">
              ...
              <button onClick={inviaOrdine} disabled={inviando}
                ...
                style={{ backgroundColor: coloreP }}>
                {inviando ? 'Invio in corso...' : '🛵 Invia ordine'}
              </button>
            </div>
          </div>
        </div>
