...
export default function PrenotaPage() {
  ...
  function selezionaSuggerimento(s: Suggerimento) {
    ...
    setCoordScelte({ lat: s.lat, lon: s.lon })
    setSuggerimenti([])
    setErrIndirizzo('')
  }
  ...
  async function inviaOrdine() {
    ...
    const errDataOraOrdine = validaDataOra(dati.data, dati.ora, preavvisoBase)
      ...
      if (dati.tipo === 'delivery') {
        ...
        if (fasce.length > 0) {
          ...
          if (fascia.preavvisoMinuti > 0) {
            const errPre = validaDataOra(dati.data, dati.ora, fascia.preavvisoMinuti)
    ...
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
          ...
          {logoUrl
            ? <img src={logoUrl} alt="" className="h-12 w-12 rounded-xl object-cover shrink-0" />
