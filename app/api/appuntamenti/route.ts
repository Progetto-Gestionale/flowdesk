...
export async function POST(req: Request) {
  ...
  if (!userId) return NextResponse.json({ error: 'Non autorizzato' }, { status: 401 })
  const user = await getOrCreateUser(userId)
  const { clienteNome, clienteEmail, servizio, data, durata, note, coperti, allergie, occasione, tavoloId, pazienteId, status } = await req.json()
  ...
  const appuntamento = await prisma.appuntamento.create({
    data: { userId: user.id, clienteNome, clienteEmail, servizio, data: dataObj, durata: durataMin, note, coperti: coperti ?? 1, allergie: allergie || null, occasione: occasione || null, tavoloId: tavoloId || null, pazienteId: pazienteId || null, ...(status ? { status } : {}) },
  })
