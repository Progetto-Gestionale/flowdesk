...
import { normQuantita, normSoglia, normEtichetta, normColore, normFoodCost, normAliquota } from '@/lib/menuPiatto'

export async function POST(req: Request) {
  const user = await getAuthUser()
  if (!user) return NextResponse.json({ error: 'Non autorizzato' }, { status: 401 })
  const { categoriaId, nome, descrizione, prezzo, foodCost, aliquotaVendita, immagineUrl, allergeni, quantita, quantitaSoglia, etichetta, etichettaColore } = await req.json()
  ...
  const piatto = await prisma.menuPiatto.create({
    data: {
      ...
      foodCost: normFoodCost(foodCost),
      aliquotaVendita: normAliquota(aliquotaVendita),
